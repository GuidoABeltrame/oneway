...
class Home extends CI_Controller {
	...
	public function index(){
		...
		$data['title'] = "AyS";
		$data['head'] = $this->load->view('modules/head', $data, TRUE);

		$data['footer'] = $this->load->view('modules/footer', $data, TRUE);
	...
	function sendContact(){
		...
		$data = array(
			...
			'info' => $this->input->post('inputCheck')
		);
	...
	function sendEmail( $data ){
		...
		for($i=0; $i<count($data['info']); $i++){
			$html .= '<p style="font-size:12px">'.$data['info'][$i].'</p>';
		}
