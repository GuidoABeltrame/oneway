...
<?php /* Initialize The Loop */ if (have_posts()) { $post = $posts[0]; // Hack. Set $post so that the_date() works. ?>
	...
	<div class="pagetitle">
		...
		Archivo para la categor&iacute;a '<?php echo single_cat_title(); ?>'

		<?php /* If this is a daily archive */ } elseif (is_day()) { ?>
		Archivo para <?php the_time('F jS, Y'); ?>

		<?php /* If this is a monthly archive */ } elseif (is_month()) { ?>
		Archivo para <?php the_time('F, Y'); ?>

		<?php /* If this is a yearly archive */ } elseif (is_year()) { ?>
		Archivo para <?php the_time('Y'); ?>

		<?php /* If this is a search */ } elseif (is_search()) { ?>
		Resultados de la b&uacute;squeda para '<?php echo $s; ?>'

		<?php /* If this is an author archive */ } elseif (is_author()) { ?>
		Archivo del autor para <?php $post = $wp_query->post;
		...
		P&aacute;gina del archivo <?php echo $paged; ?>
	...
	<span class="chronodata">
					...
					<?php foreach((get_the_category()) as $cat) {  if ($cat->cat_name == 'Noteworthy') { ?>
						<span class="metalink favorite"><img src="<?php bloginfo('template_url'); ?>/images/favorite.gif" alt="Art&iacute;culo favorito" /></span>
					<?php } } ?>
				...
				<div class="itemtext">
					<?php the_content("Seguir leyendo '" . the_title('', '', false) . "'"); ?>
				</div>
<small class="metadata">
						<span class="category">Archivado en: <?php /* If 'Nice Categories' plugin is installed, use it; if not, use default */ if (function_exists('the_nice_category')) { the_nice_category(', '); } else { the_category(', '); }?>	</span>&nbsp;&nbsp;|&nbsp;&nbsp;<?php comments_popup_link('0&nbsp;<span>Comentarios</span>', '1&nbsp;<span>Comentario</span>', '%&nbsp;<span>Comentarios</span>', 'commentslink', '<span class="commentslink">Cerrados</span>'); ?>

						
						<?php if (is_single() && function_exists(UTW_ShowTagsForCurrentPost)) { ?>
							<span class="tagdata">Etiquetas: <?php UTW_ShowTagsForCurrentPost("commalist") ?></span>
						<?php } ?>
		...
		<?php  /* Normal Entries */ } elseif (!(in_category($k2asides))) { ?>
			...
			<div id="post-<?php the_ID(); ?>" class="item entry">
				<div class="itemhead">
					<h3><a href="<?php the_permalink() ?>" rel="bookmark" title='Enlace permanente a "<?php strip_tags(the_title()); ?>"'><?php the_title(); ?></a></h3>
...
<div class="chronodata"><?php { the_time('dMy') ?><?php } ?>
				...
				<div class="itemtext">
					<?php if (is_archive() or is_search()) { 
						...
						the_content("Seguir leyendo '" . the_title('', '', false) . "'");
					} ?>
...
<small class="metadata">			
<span class="category">Archivado en: <?php /* If 'Nice Categories' plugin is installed, use it; if not, use default */ if (function_exists('the_nice_category')) { the_nice_category(', '); } else { the_category(', '); }?>	</span>

					&nbsp;&nbsp;|&nbsp;&nbsp;<?php comments_popup_link('0&nbsp;<span>Comentarios</span>', '1&nbsp;<span>Comentario</span>', '%&nbsp;<span>Comentarios</span>', 'commentslink', '<span class="commentslink">Cerrados</span>'); ?>
