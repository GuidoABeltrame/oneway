...
/*
- OBC_EXPIRATION: Expiración de la caché: número de segundos (3600 por defecto = 1 hora)
- OBC_CACHE_USERS: Comportamiento del plugin con los usuarios registrados (y logueados):
	* 0: No cachear
	* 1: Usar una única caché global
	* 2: Usar una caché individual para cada usuario (2 por defecto)
- OBC_CACHE_COMMENTERS: Comportamiento del plugin con los comentadores: mismos valores que en OBC_CACHE_USERS (2 por defecto).
- OBC_CACHE_ERROR_PAGES: Opción de cachear las páginas de error (status 404): true o false (false por defecto).
- OBC_CACHE_REDIRECTIONS: Opción de cachear las redirecciones (status 301 y 302): true o false (false por defecto).
- OBC_AVOID_TRAILING_SLASH_DUPLICATION: Opción de omitir la barra final ("/") de las urls para evitar cachear dos veces el mismo contenido: true o false (false por defecto).
	* IMPORTANTE: No establezcas este parámetro a true en WordPress 2.3+ o si estás usando un plugin que redirecciona a las urls con barra final.
- OBC_ENABLE_BROWSER_CACHE: Opción de permitir que el navegador de los visitantes cachee los contenidos: true o false (false por defecto).
- OBC_LOOK_FOR_DYNAMIC_CODE: Opción de buscar código dinámico (comentarios mfunc y mclude): true o false (true por defecto).
	* IMPORTANTE: Si tienes habilitada la compresión Gzip y no usas código dinámico, establecer OBC_LOOK_FOR_DYNAMIC_CODE a false permitirá al plugin ahorrarse el tener que leer el código cacheado en texto plano y así devolver directamente el contenido comprimido.
- OBC_USE_CACHE_DIRECTORIES: Opción de guardar los archivos de caché en una estructura de directorio que emula las urls del blog: true o false (true por defecto).
- OBC_REJECTED_STRINGS: Si la url contiene una de esas cadenas (separadas por comas), esa url no será cacheada (por defecto: "wp-").
...
- OBC_REJECTED_USER_AGENTS: Si el user-agent (nombre del navegador, bot, etc. del cliente) contiene una de esas cadenas (separadas por comas), sus peticiones no serán cacheadas, aunque las páginas cacheadas se mostrarán si existen y no han expirado (por defecto: "bot, ia_archive, slurp, crawl, spider").

// DEJA DE EDITAR DEBAJO DE ESTA LÍNEA!
*/
